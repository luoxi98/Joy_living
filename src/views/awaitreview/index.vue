...
import { receipt_indent } from "../../api/receipt_indents";
...
export default {
  ...
  methods: {
    ...
    confirm_active(item) {
      this.$confirm("确认全部收到货了？").then(() => {
        receipt_indent({ indent_collection: item[0].indent_collection }).then(() => {
           this.$message("收货成功");
            //   history.go(0);
        });
      });
