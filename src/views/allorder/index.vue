<template>
  <div class="awaitshipments">
    ...
    >
      ...
      <div class="indent_item_footer" v-if="item[0].orderstatus =='待发货'"> <!--待发货-->
        <el-button @click="handleClose(item)">取消订单</el-button>
      </div>
      <div class="indent_item_footer" v-if="item[0].orderstatus =='待付款'"> <!--待付款-->
        <el-button>取消订单</el-button>
        <el-button type="danger" @click="payindent(item)">去付款</el-button>
      </div>
        <div class="indent_item_footer" v-if="item[0].orderstatus =='待收货'"><!--待收货-->
        <el-button type="danger" @click="handleClose(item)">取消订单</el-button>
        <el-button @click="confirm_active(item)">确认收货</el-button>
      </div>
    </div>
  </div>
</template>
<script>
import { all_order } from "../../api/all_indent";
